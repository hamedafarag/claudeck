...
describe("setWss", () => {
  it("stores the WSS instance for broadcasting", () => {
    const mockClient = { readyState: 1, send: vi.fn() };
    const mockWss = { clients: new Set([mockClient]) };
    setWss(mockWss);

    logNotification("agent", "Test Title");
    expect(mockClient.send).toHaveBeenCalledTimes(1);
  });
});

describe("logNotification", () => {
  it("calls createNotification with all arguments", () => {
    logNotification("agent", "Agent done", "5 turns", '{"cost":0.01}', "sid-1", "agent-1");
  ...
  it("returns the created notification object", () => {
    const result = logNotification("agent", "Test");
    expect(result).toHaveProperty("id", 1);
    expect(result).toHaveProperty("type", "agent");
  });

  it("passes null for optional arguments", () => {
    logNotification("error", "Failed");
  ...
  it("does not throw when wss is null", () => {
    setWss(null);
    expect(() => logNotification("agent", "Test")).not.toThrow();
    expect(createNotification).toHaveBeenCalled();
  });
});

describe("broadcastReadUpdate", () => {
  it("broadcasts notification:read with ids and updated unread count", () => {
    const client = { readyState: 1, send: vi.fn() };
    const mockWss = { clients: new Set([client]) };
    setWss(mockWss);

    broadcastReadUpdate([1, 2, 3]);
  ...
  it("broadcasts with empty ids array for mark-all-read", () => {
    const client = { readyState: 1, send: vi.fn() };
    const mockWss = { clients: new Set([client]) };
    setWss(mockWss);
  ...
  it("does not throw when wss is null", () => {
    setWss(null);
    expect(() => broadcastReadUpdate([1])).not.toThrow();
  });

  it("fetches fresh unread count from DB", () => {
    const client = { readyState: 1, send: vi.fn() };
    setWss({ clients: new Set([client]) });

    broadcastReadUpdate([1]);
